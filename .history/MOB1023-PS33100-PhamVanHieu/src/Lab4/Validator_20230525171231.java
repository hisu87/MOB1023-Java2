...
public class Validator {
    ...
    public static boolean isNumberGreaterZero(JTextField textField, StringBuilder sb, String msg, int type) {
        if (!isNumber(textField, sb, msg, type)) {
            return false;
        } else {
            msg = "Số nhập phải lớn hơn 0";
            double number = Double.parseDouble(textField.getText().trim());
            if(number < 0){
                sb.append(msg).append("\n");
                textField.setBackground(Color.yellow);
    ...
    public public static void sub10() {
        int i;
        while(i>10){
            for(i = 10; i > 0; i--){
